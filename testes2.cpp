...
public:
  	...
  	void initialize(int);
  	void preenche();
  	void insertEdge(Vertex, Vertex, float);
  	void print();
  	void destroy();
...
void Grafo::insertEdge(Vertex u, Vertex v, float peso) {
...
public:
    Sistema(){};
    Sistema(Vertex valor): valor(valor) { }
...
void dfsVisita(Grafo &g, Item &u){
	...
	for(int i = 0; i < u.getVertices().size(); i++){
		...
		if(g.getAdj()[vertice_adj].getCor() == BRANCO){ //somar na ida quando visita?
			//soma ?
	...
	u.setCor(PRETO); //somar na volta com o predecessor ?
	tempo = tempo+1;
	u.setF(tempo);
}
...
int main(int argc, const char * argv[]) {
  ...
  caminho.mostra();*/
	int ordem, tamanho; Vertex vertice_1, vertice_2;
	float peso;
	...
	for(int i = 1; i <= tamanho; i++){
		cin >> vertice_1; cin >> vertice_2; cin >> peso;
		grafo->insertEdge(vertice_1, vertice_2, peso);
	}

	Sistema s(1);
	s.setGrafo(grafo);
	cout << s.getVertex() << " : " << endl;
	s.getGrafo()->print();
	//grafo.print();
	cout << endl << "fim";
